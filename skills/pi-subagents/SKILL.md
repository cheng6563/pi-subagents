...
默认最大深度 1。仅在当前授权和项目规则允许时显式设置 `maxDepth` 开启后代；后代继承剩余深度且不能增加上限。不能通过 CLI、脚本或 SDK 包装绕行。工具命令检查不是操作系统沙箱。
...
`interrupt` 暂停可恢复，`cancel` 终止不可恢复。子代理用 `action: "report"` 向父线程报告，父线程用 `steer` 补充信息。不要因失败状态自动重做已完成的外部操作。
