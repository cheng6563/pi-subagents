...
这是基于原 pi-subagents 运行时代码裁剪、独立维护的实现。保留 MIT 许可及历史提交，不常规合并或同步上游；只按需评估具体修复。提供通用 `subagent` 工具及 `/subagents` 界面。工具定义自包含启动、管理、恢复和参数规则，常规调用无需先读本文件；本文件用于详细说明、维护与排障。不附带重复的用法 skill；任务方法与业务标准由调用方按需提供。
...
- `task` 首句写动作、目标对象和目的，供界面摘要使用；背景、授权与约束放在后面，不以委派缘由开头。每次启动相互独立，并发修改共享文件时由调用方协调，避免覆盖。
