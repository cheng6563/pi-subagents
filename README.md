...
交互模式的调用卡片显示任务简述，启动回执保持静态。进度位于输入区下方固定 8 行区域，以父子树展示仍有活动运行的任务组，组内已结束的后代保留状态。每个节点只占一行：状态、任务首句截短、`tools 3 · $0.03`、最新消息末段；长行限宽，不自动换行。统计仅计该运行自身，费用取运行时记录的估算值；缺失显示 `—`，不表示服务商账单或剩余额度。节点超出可用行数时显示剩余数量，通过 `/subagents` 查看全部。
