...
let tool: any;
const notices: any[] = [];
registerExecutor({ registerCommand() {}, registerMessageRenderer() {}, registerEntryRenderer() {}, registerTool: (t: any) => { tool = t; }, getThinkingLevel: () => process.env.PI_REASONING_LEVEL || "off", on: (name: string, cb: any) => { if (name === "session_shutdown") handlers.push(cb); }, sendMessage: (message: any) => notices.push(message) });
