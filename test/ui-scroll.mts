...
import { createRun, readContract, saveRun, storeRoot, writeJson } from "../src/store.ts";
import { emptyProgress, createViewReader } from "../src/progress.ts";
...
const { createRunResultRenderer, renderCompletionCard } = await jiti.import<any>("../src/ui.ts");
...
const run = createRun(storeRoot(session.getSessionId()), { version: 1, task: "SCROLL_PROBE", cwd: root, model: { provider: "fixture", id: "parent", thinking: "off" }, context: "fresh", depth: { depth: 1, maxDepth: 3 }, timeoutMs: 60000 });
...
try {
  ...
  transcript.addChild(new tui.Text(Array.from({ length: 160 }, (_, i) => `HISTORY_${String(i).padStart(3, "0")}`).join("\n"), 0, 0));
  screen.addChild(transcript);
  screen.addChild(new tui.Text("EDITOR", 0, 0));
  const originalReceipt = card.render(80);
  ...
  writes = [];
  transcript.addChild(renderCompletionCard({ view: createViewReader()(run), output: "FINAL_SCROLL_RESULT" }, false, theme));
  screen.renderNow();
  const appendClears = writes.join("").includes("\x1b[3J");
  const result = { heights, runningClearsScrollback: runningClears, completedClearsScrollback: completedClears, appendClearsScrollback: appendClears };
  ...
  if (!process.argv.includes("--probe")) {
    ...
    assert.deepEqual(card.render(80), originalReceipt, "The original receipt must never be replaced after completion");
    assert.equal(appendClears, false, "Appending a completion at the tail must preserve existing scrollback");
  }
...
const child = createRun(join(run.dir, "children"), { ...readContract(run), task: "CHILD_PROBE", depth: { depth: 2, maxDepth: 3 } });
const grandchild = createRun(join(child.dir, "children"), { ...readContract(run), task: "GRANDCHILD_PROBE", depth: { depth: 3, maxDepth: 3 } });
const peer = createRun(storeRoot(session.getSessionId()), { ...readContract(run), task: "PEER_PROBE" });
const otherRuns = [child, grandchild, peer].map(v => ({ path: join(v.dir, "status.json"), progress: join(v.dir, "progress.json"), output: v.outputPath, task: readContract(v).task }));
...
writeJson(join(root, "fixture.json"), { root, agentDir, otherRuns, session: session.getSessionFile(), activeRun: join(run.dir, "status.json"), output: run.outputPath, progress: join(run.dir, "progress.json"), extension: fileURLToPath(new URL("../index.ts", import.meta.url)), cli: join(host, "dist/cli.js") });
