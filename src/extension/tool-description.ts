...
Model override: use shared:<name> to resolve an entry from the global shared-models.json at launch (e.g. shared:lowCost), or first call {action:"models"} and copy an exact provider/id, not an agent name. Shared references select a model, not an agent or a workflow. Thinking uses model suffix, not watchdog-only thinking.
